...
# 映射成[[news1.boolean, news1.boolean, ...], [news2.boolean, news2.boolean, ...], ...]
def create_set_representation(news_shingles_rdd):
    ...
    all_shingles = all_shingles_rdd.collect()
    set_representation_rdd = news_shingles_rdd.map(lambda each_news_shingles: create_set_row(each_news_shingles, all_shingles))
    set_representation_rdd = all_shingles_rdd.union(set_representation_rdd)
...
# 對單一新聞，遍歷所有shingle並判斷目標新聞中是否出現該shingle(以新聞為列)
def create_set_row(each_news_shingles, all_shingles):
    temp_row = []
